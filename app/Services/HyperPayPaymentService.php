...
class HyperPayPaymentService
{
    ...
    //     <style>
    //     .wpwl-container {
    //         max-width: 600px;          /* حد أقصى للفورم */
    //         margin: 50px auto;         /* يترك مسافة من الأعلى ويثبت في الوسط */
    //         background-color: #fff;    /* خلفية بيضاء للفورم */
...
public function get_view(string $id, string $method): string
{
    ...
    $brands = $method;
        $offers = Offers::all();
    return view('payment', compact('id', 'method', 'baseUrl', 'brands','offers'))->render();
}
    ...
    public function checkPaymentStatus()
    {
        ...
        return $this->handle('checkouts/' . $id . '/payment', env('HYPER_PAY_TOKEN','OGFjOWE0Y2Q4YzNlNjdlYjAxOGM2NzQxNzY2MDYzNDh8bWpUWlc5VHp0ZGVDSzdjZw=='), [
            'entityId'=>$entityId
        ], 'get');
    ...
    {
        $entityId = env('visaMasterEntityId','8ac9a4cd8c3e67eb018c67420060634d');
        if ($method == 'MADA') {

            $entityId = env('madaEntityId','8ac9a4cd8c3e67eb018c674304416354');
    ...
    {
        $token = env('HYPER_PAY_TOKEN','OGFjOWE0Y2Q4YzNlNjdlYjAxOGM2NzQxNzY2MDYzNDh8bWpUWlc5VHp0ZGVDSzdjZw==');
