...
if ($status !== null && $status !== '') {
    $baseQuery->where('status', $status);
            ...
            // آخر 7 أيام
            $last7DaysStats = $calc(
                (clone $baseQuery)->whereBetween('date', [Carbon::now()->subDays(6)->toDateString(), $today->toDateString()])
            );
    
            // هذا الأسبوع
            $thisWeekStats = $calc(
                (clone $baseQuery)->whereBetween('date', [$weekStart->toDateString(), $weekEnd->toDateString()])
            );
    
            // هذا الشهر
            $thisMonthStats = $calc(
                (clone $baseQuery)->whereBetween('date', [$monthStart->toDateString(), $monthEnd->toDateString()])
            );
    
            // الشهر الماضي
            $lastMonthStats = $calc(
                (clone $baseQuery)->whereBetween('date', [$lastMonthStart->toDateString(), $lastMonthEnd->toDateString()])
            );
            ...
            $onlineOrders = $totalOrdersPayments - $branchOrders;
    
            $branchAmount = (clone $paymentsQuery)
                ->where('payment_method', 'payBranch')
                ->sum('total_price');
    
            $totalAmountRange = $customStats->total_amount;
            ...
            $paymentsBreakdown = [
                ...
                'branch_orders_ratio'   => $totalOrdersPayments ? ($branchOrders / $totalOrdersPayments) * 100 : 0,
                'online_orders_ratio'   => $totalOrdersPayments ? ($onlineOrders / $totalOrdersPayments) * 100 : 0,
    
                'branch_amount'         => $branchAmount,
                'online_amount'         => $onlineAmount,
                'branch_amount_ratio'   => $totalAmountRange ? ($branchAmount / $totalAmountRange) * 100 : 0,
                'online_amount_ratio'   => $totalAmountRange ? ($onlineAmount / $totalAmountRange) * 100 : 0,
            ];
            ...
            $customerMix = [
                ...
                'new_ratio'           => $uniqueCustomersCount ? ($newCustomersCount / $uniqueCustomersCount) * 100 : 0,
                'returning_ratio'     => $uniqueCustomersCount ? ($returningCustomersCount / $uniqueCustomersCount) * 100 : 0,
            ];
            ...
            if ($yesterdayStats->total_amount > 0) {
                $growthTodayVsYesterday = (($todayStats->total_amount - $yesterdayStats->total_amount) / $yesterdayStats->total_amount) * 100;
            }
    
            $growthThisMonthVsLastMonth = null;
            if ($lastMonthStats->total_amount > 0) {
                $growthThisMonthVsLastMonth = (($thisMonthStats->total_amount - $lastMonthStats->total_amount) / $lastMonthStats->total_amount) * 100;
            }
        ...
        return redirect()->route('admin.dashboard.branch', $admin->branch_id);
    }
    
    public function dashbook(Request $request)
    {
        // جلب الفروع للفلتر وللعرض
        $branches = Branch::orderBy('branch_name_ar')->get();

        // قيم الفلتر من الريكوست
        $branchId = $request->get('branch_id');
        $from     = $request->get('from');
        $to       = $request->get('to');
        $sort     = $request->get('sort', 'desc'); // desc = الأحدث أولاً (افتراضي)
        $status = $request->get('status');


        // كويري أساسي مع علاقة الفرع
        $query = Wash_booking::with('branch');
...
if ($status !== null && $status !== '') {
    $query->where('status', $status);
        ...
        // تجميع الحجوزات حسب الفرع
        $bookingsByBranch = $bookings->groupBy('branch_id');

        // ===== نفس منطق customFrom/customTo بتاع الـ Dashboard =====
        $today = Carbon::today();
